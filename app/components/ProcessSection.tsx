...
function processTextMotionProps(reduceMotion: boolean, delay = 0) {
  return {
...
function ProcessCardBody({ reduceMotion }: { reduceMotion: boolean }) {
  return (
    <>
      <h2 className="sr-only">Process</h2>
      <ProcessList reduceMotion={reduceMotion} />
    </>
  );
}
        ...
        >
          <div className="flex items-start gap-4 sm:gap-6 lg:contents">
            ...
            <motion.h3
              ...
              {...processTextMotionProps(reduceMotion, index * 0.04)}
            >
              {row.title}
            </motion.h3>
          </div>
          ...
          <div className="flex min-w-0 flex-col gap-3 sm:gap-4 lg:max-w-[min(52ch,100%)] lg:pr-4">
            <motion.p
              className="font-ui m-0 text-pretty text-[13px] font-normal leading-[1.58] text-cream/72 sm:text-sm md:text-[15px] md:leading-relaxed"
              {...processTextMotionProps(reduceMotion, index * 0.04 + 0.08)}
            >
              {row.description}
            </motion.p>
...
export default function ProcessSection() {
  ...
  return (
    <section
      id="process"
      className="pt-10 pb-20 md:pt-12 md:pb-28 lg:pt-14 lg:pb-32"
      style={{ background: SECTION_TAN, color: "#171717" }}
    >
      <div className="w-full px-2 sm:px-3 md:px-4">
        {reduceMotion ? (
          <div className="mx-auto max-w-[1400px]">
            <div className={processCardInnerClassName}>
              <ProcessCardBody reduceMotion />
            </div>
          </div>
        ) : (
          <ContainerScroll
            containerClassName="relative flex min-h-[30rem] w-full items-center justify-center py-2 md:min-h-[40rem] md:py-4"
            perspectiveClassName="relative w-full"
            cardClassName="mx-auto h-auto min-h-0 w-full max-w-[1400px] border-0 bg-transparent p-0"
            cardInnerClassName={processCardInnerClassName}
            flat
            scrollOffset={["start 0.9", "end 0.24"]}
          >
            <ProcessCardBody reduceMotion={false} />
          </ContainerScroll>
        )}
      </div>
    </section>
